...
### Task 1 — Fail-closed guard for an empty scope-check result, with distinguishable UNREVIEWED causes
...
Done means:
1. [cap]  an empty-but-non-null `files` array degrades the product-review lane to `UNREVIEWED` exactly like a died scope-check agent, capping the compiled verdict at `HOLD`   (tier: test-backed `tests/python/test_acceptance_fanout.py`)
2. [cap]  the compiled `summary` text distinguishes an empty-changeset cause from an agent-death cause for the same `missing`-lane entry, never both reading identically   (tier: test-backed `tests/python/test_acceptance_fanout.py`)
3. [hold] every pre-existing test in `tests/python/test_acceptance_fanout.py` still passes, with only cause-text string assertions updated to the new convention and no behavioral change   (tier: test-backed `tests/python/test_acceptance_fanout.py`)
...
Done means:
1. [cap]  a story whose changeset contains exactly one `docs/studious/premortems/*.md` file dispatches `@agent-premortem-auditor` (lane `product`) inside the same `parallel()` round as product-review and walkthrough, not after it resolves   (tier: test-backed `tests/python/test_acceptance_dispatch_fix.py`)
2. [cap]  the premortem-auditor's `REALIZED` findings appear in the compile prompt sent to `acceptanceFanIn` as a distinct third block, separate from the product-review and walkthrough blocks   (tier: test-backed `tests/python/test_acceptance_dispatch_fix.py`)
3. [cap]  a register containing only technical-lane items still triggers the `@agent-premortem-auditor` dispatch inside `acceptanceRound` — the decision is presence-only, never content-inspecting   (tier: test-backed `tests/python/test_acceptance_dispatch_fix.py`)
4. [hold] a story whose changeset contains no premortem file dispatches no premortem-auditor call and behaves identically to today's fan-out   (tier: test-backed `tests/python/test_acceptance_dispatch_fix.py`)
Evidence: pytest output; `node --check workflows/epic-driver.js`; `npx eslint@10.6.0 workflows/` clean.
...
Done means:
1. [cap]  a changeset with zero premortem files but one `Branch:`-matching file elsewhere under `docs/studious/premortems/` still gets verified via the fallback path   (tier: test-backed `tests/python/test_acceptance_dispatch_fix.py`)
2. [cap]  a died or unparseable fallback dispatch degrades the lane to `UNREVIEWED` with a reason distinct from "confirmed absence," never silently treated as "no register exists"   (tier: test-backed `tests/python/test_acceptance_dispatch_fix.py`)
3. [hold] a changeset with zero premortem files and a genuinely confirmed empty `docs/studious/premortems/` directory still skips verification exactly as before this story   (tier: test-backed `tests/python/test_acceptance_dispatch_fix.py`)
...
Do:         when the `Branch:`-header filter still leaves more than one candidate register — whether the changeset itself names more than one, or the fallback does — degrade the lane to `UNREVIEWED` with its own distinguishable reason via Task 1's convention, never picking one arbitrarily or guessing.
...
Done means:
1. [cap]  two `Branch:`-matching candidate registers (from the changeset, the fallback, or both) degrade the lane to `UNREVIEWED` with a reason distinct from every other cause, never resolved by picking one   (tier: test-backed `tests/python/test_acceptance_dispatch_fix.py`)
2. [hold] the single-candidate case from Task 2 and the zero-candidate cases from Task 3 are unaffected by this task's own changes   (tier: test-backed `tests/python/test_acceptance_dispatch_fix.py`)
