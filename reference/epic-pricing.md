...
`/next`'s plan piece stops once, for the user to approve a story plan. This file
prices that stop: how to estimate the approved plan's cost, and how to propose the
appetite the driver then enforces as a ceiling. Lookup data read at plan approval —
nothing in `workflows/epic-driver.js` reads it, and nothing here is recorded except
the two approved numbers.
...
Nobody chose to spend 22M tokens — the run had no ceiling because the price was never
visible at a moment anyone could act on it. Approval is that moment.
...
history below is recorded in. Convert to dollars for the human if it helps, but **the
number recorded as the appetite is always tokens.** A dollar figure recorded as the
appetite would be a rate table stored in a ledger, going stale silently against every
price change.
...
1. **Measured, from this project's own history.** The store is
   ...
   [#296](https://github.com/jacquardlabs/studious/issues/296) asks for; it is partial
   today because the telemetry store records *which* dispatches went out (one
...
2. **The pinned floor.** No usable history — a first epic, a fresh checkout, a project
   whose telemetry predates joinable identity. Use the per-phase table below, labeled
   a floor estimate, provisional.

Never estimate from a model's own guess at what it will spend: models cannot predict
their own token usage (correlation ≤ 0.39, arXiv:2604.22750) and identical tasks vary
up to 30x — why the appetite proposed below is a p99, not a mean.
...
the ceiling is denominated in tokens:
...
top of the range computed above. Anthropic's task-budget guidance sizes budgets from
measured distributions starting at p99: a budget set too small doesn't fail loudly, the
work quietly scopes down instead, reading as a weaker result rather than a ceiling
being hit. **Say that out loud whenever the user proposes a tighter number than the
estimate** — a tight appetite is a legitimate choice, but a choice to accept a degraded
result, not a smaller one.
...
may be awaiting judgment or human action at once. Practitioner reports converge on 1–4
as the effective concurrency where a human is in the loop; M11 demonstrated the failure
mode from the other side — 22M tokens, 7 invocations, one person absorbing 21 fix-round
verdicts. Token headroom does not bind at ship time; review bandwidth does. A story
parked by the plan as `story-supervised` counts against this number exactly like one
parked on a gate verdict — both are work the person has to come back to.

## Show the estimate the way the profile was shown

The gate profile is presented with the inputs that produced it
(`reference/epic-plan-contract.md`); price follows the same rule: per-story lines, the
rung the multiplicand came from, and the range — never a bare total. A user can't trim
a plan toward a number they like if the number arrives without its parts.
...
- `workflows/epic-driver.js` — enforces at runtime and prices nothing (computing a rate
  here would be a second source of truth for cost). The two numbers are **not**
