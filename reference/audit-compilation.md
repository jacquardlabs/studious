...
Every judge lane returns a findings document whose `tier` is already `critical`, `important`, or `track` (gauntlet's `docs/findings-contract.md` §5), and gauntlet's `scripts/report.py` applies anchor-or-demote and taste-caps-at-track at ingest, naming each change it made — nothing is mapped here. The one lane outside that path is `/review`'s inline `web-design-guidelines` run (lane 8, skill installed), whose labels map through the a11y row in `reference/severity-rubric.md`. `auditFanIn()` dispatches the same judges (#334 S2) but cannot run `report.py`, so `workflows/epic-driver.js` applies the two changeset ingest rules itself in code (`normalizeFindings`) and renders each change as an `ingest:` line under the lane's block; its prose lanes — the cross-lane spot-check on a narrowed retry, the finale's closure and seam lanes — are placed on the same three tiers by the compiler. Either way, consult it, don't restate it.
