...
Unlike the three states above, this fourth one applies to exactly one caller. Accessibility (`/gate-audit`'s auditor 8) is not a member of `workflows/epic-driver.js`'s `AUDITORS` roster at all — the epic driver has no way to detect, from inside a Workflow script, whether the session consuming its output has the optional `web-design-guidelines` skill installed, so it never dispatches either of auditor 8's two paths (issue #274 tracks whether that should change). This is fixed, not data-driven: it is not a per-round routing decision the way the other three states are, so it never varies round to round, and every compiled report `auditFanIn()` produces — story or finale — renders it as `studious:accessibility-auditor --- (not covered on the epic path: ...)`. Treat it as neutral, exactly like routed-out: never a gap that blocks the verdict, never conflated with AGENT DIED (its absence is a standing coverage decision, not a dispatch failure that happened this round), and never something to raise as a finding in its own right. `/gate-audit`'s own interactive session never produces this state at all — its auditor 8 always runs one of its two paths (the installed skill, or the Task fallback), so accessibility is never simply absent there. `auditFanIn()` injects its own fixed Summary-line instruction for this state, the same way it does for routed-out — this file gives `/gate-audit`'s own session no equivalent instruction, since the state cannot arise there.
