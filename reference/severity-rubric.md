# Severity rubric — canonical tiers and the one row studious still maps

Canonical three-tier severity ladder for `/review`. The judge lanes are gauntlet's and emit
these tiers directly (its findings contract, `docs/findings-contract.md` §5), so no
...
studious states itself is the inline lane's:
