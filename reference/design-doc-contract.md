...
This file is the sole authority for what a design doc must contain, and it is deliberately producer-agnostic: a doc satisfies the contract if @agent-product-reviewer's design-mode review (`agents/product-reviewer.md`) can answer its seven questions from the doc's text alone, whoever wrote it. `/design` ships in this plugin and is one implementation; a third-party planning tool's output or a hand-written spec satisfies it identically. That framing is load-bearing — `/gate-design-review` judges the work, never who produced it (`scripts/check_gate_independence.py`), the same rule `reference/worker-contract.md` holds for the build phase. Originally specified in [#29](https://github.com/jacquardlabs/studious/issues/29); the section set was reconciled across its four copies in [#211](https://github.com/jacquardlabs/studious/issues/211).
