...
| Evidence & reports | Two runs of one query verb, one per stream — the rows: `scripts/evidence-capture list --repo <worktree> --branch "$(git -C <worktree> rev-parse --abbrev-ref HEAD)" 2>/dev/null`, then the notes: `scripts/evidence-capture list --repo <worktree> --branch "$(git -C <worktree> rev-parse --abbrev-ref HEAD)" 2>&1 >/dev/null`. The first gives one `<task>` + answer line per task, exit 0 even with no rows; never a glob over `docs/jig/evidence/`, whose folder names this skill does not parse. `--repo` is not optional here: it defaults to `.`, so an omitted flag reads whichever checkout the session's cwd happens to sit in rather than the one the build ran in. `--branch` takes that exact command because it is the one the capture stamped the manifest with, literal `HEAD` fallback included; `git branch --show-current` prints an empty string on a detached checkout and matches no row. Reports: `docs/jig/reports/` (Glob) | Which tasks have evidence — a row is a capture that resolved for this branch, not proof this branch produced it. The answer column is a path, optionally opened by a bracketed token, and a token is the verb's own qualification of that row: quote it as printed and never interpret it, since its meaning is the script's to state and it states it once — on stderr, one line per row it has anything to say about, whether that row carries a path or only a token. Read that stream too — that is what the second run above is for, and why the invocation is not a single bare command: one run hands you the two streams merged, where a note reads as a row and a row reads as a note. Two runs rather than `/finish`'s `2> <scratch-path>/resolve-note.txt` redirection because this skill writes no file; the cost is one extra run of a verb that changes nothing. Each note names its own task (`the folder resolved for task '<id>'` where the row carries a path, `no folder resolved for task '<id>'` where it carries only a token), which is the key that joins it back to the row it qualifies — never position, since the two runs print independently. Put each note where the assessment already has room for it: verbatim on that row's own evidence line (below), one note per noted row, never merged across rows and never carried up into the recommendation. Write no gloss of your own. A row whose answer column is the literal `[ambiguous]` instead of a path carries a note of its own on that same stream — quote that note the same way and for the same reason, since what the token means there is the script's to state too. Report it as unevidenced *and* name it — it is the same state `/finish` labels "evidence ambiguous" — and never treat one of those folders as this branch's. On a task the **Task statuses** row above shows as not yet terminal, that row is about the task *id*, not about this task's evidence: nothing has been built for it yet, so what the verb could not pick between was inherited from earlier work sharing the number — say that, rather than reporting an ordinary todo as an evidence problem. **An id with no status row at all takes the same reading**, and is the easier one to misreport because there is no todo on screen to attribute it to: when no `### Task` block in this branch's `PLAN.md` names that id, the row is about work this branch never planned, so say the id belongs to no task here rather than reporting it as an evidence problem this branch has. A token *followed by* a path is a qualified answer rather than an absent one: report the path with its token attached, and never drop the token to make the row read cleanly — and where such a row sits on either kind of non-terminal task, that path is still earlier work's, so never let it read as this branch having evidence. If the verb cannot be run at all — script missing, invocation failing — name it in the assessment and mark this signal unread ("couldn't run `evidence-capture list`; evidence signal unread"), the same shape the unreadable-ledger rule below takes. Never fall back to a glob, and never drop the line. Whether a story already closed out via `/finish` (a dated build report). |
