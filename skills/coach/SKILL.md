...
| Evidence & reports | Evidence: `docs/jig/evidence/*/manifest.json` (Glob), then Read each manifest's own `branch` and `task` fields. The folders are named `docs/jig/evidence/<date>-<task>-<branch-slug>/` (`scripts/evidence-capture`'s own `target_dir` grammar, branch-slugged since #258) — that is what they look like, never a name to reconstruct: `<date>` is the capture date rather than today, and the slug collapses every `/` to `-`. This branch's evidence is the folders whose manifest `branch` equals the current branch, read with `git rev-parse --abbrev-ref HEAD` — the same command `scripts/_gitutil.py`'s `current_branch` stamped the manifest with, literal `HEAD` fallback included, so a detached checkout still names the branch the writer's way; `git status` prints `HEAD detached at <sha>` there and matches no manifest (`skills/finish/SKILL.md` spends a paragraph on this same hazard for `--show-current`). Match the branch *name*, never its slug, because `feat/foo` and `feat-foo` share one slug. A manifest carrying no `branch` predates #258 — don't claim it for this branch, and don't let it disappear into the empty case either: report it on its own line, "N evidence folder(s) predate branch-stamped capture and carry no `branch` field; not claimed for this branch," naming the folders when the count is small enough to read and giving the count alone when it isn't. Give that same line the way out rather than ending at the condition: print the `resolve` command below with it, since a lone branch-less folder matching the task is exactly the case that command answers. That line is what the human reads, so it names the *condition* and never an issue number: the reader is a developer in their own repo with no access to this plugin's tracker, and "#258" is a fact they cannot act on. **That refusal is deliberately stricter than the script's own rule.** `scripts/evidence-capture`'s `resolve`/`list` — the one home for branch-to-folder attribution, and what `/finish` actually calls — answers a task three ways this row declines to: a lone branch-less folder matching the task resolves with a `[legacy]` token, two or more return `[ambiguous]` naming the candidate folders, and several branch-bearing matches resolve newest-first by `captured_at`. On disagreement that script is authoritative and this read is not; say so, and point the human at `scripts/evidence-capture resolve --repo <worktree> --branch "$(git -C <worktree> rev-parse --abbrev-ref HEAD)" --task <N>` rather than running it yourself (see "Does no work itself") — hand over that whole form, the one `skills/finish/SKILL.md` already runs, with `<worktree>` and `<N>` already substituted from what Step 1 read, never as a shape the reader has to fill in. Never trim it down to the task alone: `--branch` is `required=True` in that script's `parse_query_args`, so any form omitting it argparse-errors with exit 2, and the escape hatch dies in the hand of a human who is already confused about attribution. Reports: `docs/jig/reports/YYYY-MM-DD-<story-slug>-build-report.md` — `scripts/build-report`'s own grammar, the whole filename, not just the folder. | Which tasks captured evidence, by manifest `task`. Report the two empty cases apart: no `docs/jig/evidence/` at all (nothing has ever been captured here) versus the folder existing with no manifest matching this branch (other stories captured, this one has not) — that second case means manifests that *carry* a `branch` naming a different one; a folder whose manifest carries no `branch` at all is the pre-#258 case above, goes on the declined line, and is never reported as another story's capture. Collapsing them is #260 — the silent miss this row's grammar exists to prevent. Whether **this** story closed out via `/finish`: only a report whose `<story-slug>` segment names this story — and the slug you match it against comes from the branch (the segment after `--` in `epic/<epic>--<story>`) or from the work file's slug with the epic prefix stripped, since `/finish`'s `--slug` is model-chosen and no read gives you the exact string it passed. Match that segment inside the filename and never reconstruct the `YYYY-MM-DD` half: it is the report's own write date, not today. Any other story's report sitting in the same folder is not this story's closeout — reports accumulate there across stories, so a folder-level hit is not the signal. No matching filename, or several plausible candidates, is an ambiguity to state by name; never read it as "already done." |
