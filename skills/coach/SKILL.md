...
| Evidence & reports | `docs/jig/evidence/<date>-<task>-<branch-slug>/` (`scripts/evidence-capture`'s own `target_dir` grammar — the branch slug has been part of it since #258, so a name rebuilt without it matches nothing), `docs/jig/reports/` | Which tasks captured evidence; whether a story already closed out via `/finish` (a dated build report). |
