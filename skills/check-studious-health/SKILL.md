...
This is the natural-language entry to `/doctor`. The user is asking whether their install is working correctly, not asking to run a specific gate or review — route that to the doctor command instead of guessing at the answer.

Invoke the `/doctor` command. Do not reimplement its checks here — the command owns them. It reports tooling presence (git/gh/jq), whether every shipped agent and skill actually registered this session, and whether context docs are missing, still template stubs, or populated.
