...
`<details>` block quoting the report's `Concepts removed:` and `Concepts kept:` lines,
then its `## Held` section verbatim when the report carries one — the route `/build`
Step 3 promises a `hold` finding, exactly as an Inspector `CONCERN` rides
`inspector:report` (see Assembling the table). Exorcist omits `## Held` entirely when
nothing was held, which is the common case: then the `<details>` block ends after the
two `Concepts` lines, and that is not a gap to fill — there is no held finding to quote.
