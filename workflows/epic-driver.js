...
const AUDITORS = [
  ...
  'studious:operability-auditor', 'studious:dependency-auditor',
...
// the prior round's recorded sha — not an eleventh registered auditor, not a blend of the
// ten specialists' full depth, an explicit bounded exception to "one agent = one
...
// canonical pattern-list file commands/gate-audit.md's own auditor 9 / 11 / 6-8
// routing rules point at, so there is exactly one list to ever drift from.
function routingScopeCheckPrompt(dir, base) {
  return `This is a mechanical fact-check, not a judgment call — apply the listed patterns exactly, never interpret or editorialize. From ${dir}: compute the merge-base with ${base} (git merge-base ${base} HEAD) and run git diff --name-only <that merge-base> HEAD to get the changed-file list. Read reference/audit-routing-signals.md from the plugin root (the Studious plugin root is dirname "$(command -v gate-ledger)")/..) for the canonical IaC/CI/deploy, frontend, and dependency file-pattern lists. Determine whether any changed file matches the IaC/CI/deploy list (infraMatch), whether any changed file matches the frontend list (frontendMatch), and whether any changed file matches the dependency manifest/lockfile list (depMatch). When a changed file only loosely or ambiguously matches a pattern, resolve that pattern's match to true, never false — the same "when ambiguous, run" bias commands/gate-audit.md's own routing rules use. Return your findings as EXACTLY one line of compact JSON, nothing else: {"infraMatch":<true|false>,"frontendMatch":<true|false>,"depMatch":<true|false>}`
}
...
// routing dispatch's {infraMatch, frontendMatch, depMatch} flags (resolveRoutingMatchFlags,
...
async function finaleAuditRound(note, priorResult) {
  // One story-slot fans out to 10 auditors + a compiler; the harness queues
  // beyond its own concurrency limit, so a cap-3 epic peaking above 11 agents
