"""Pins for `/health`'s dispatch and compile: gauntlet's commands, never a hand-inlined copy.

`commands/health.md` mirrors `commands/review.md`'s "Locate gauntlet", "Build the
invocations", "Dispatch", and "Compile" steps — `gauntlet dispatch` builds and validates
every invocation (and resolves each judge's standard from gauntlet's charter), `gauntlet
report` applies the ingest rules. A restated invocation or ingest rule here is the
divergence this file exists to catch: the last hand-inlined copy had already dropped
`standard.version` and schema validation. Which flags gauntlet accepts is
`test_gauntlet_contract.py`'s; the context-files and untrusted-content pins are
`test_door_posture.py`'s.
"""
...
def test_the_locate_step_cites_reviews_and_restates_no_check() -> None:
    """#410/#441: the discovery check lives once, in review.md's 'Locate gauntlet' (which
    names `studious gauntlet-contract`); health cites that step, stop included."""
    locate = _section(_door(), "## Locate gauntlet", "## Resolve the artifact")
    assert "`commands/review.md`'s \"Locate gauntlet\"" in locate
    assert "or stop, the same way" in locate
    for restated in ("command -v gauntlet", "gauntlet-contract", "/plugin install"):
        assert restated not in locate, f"health restates review's discovery: {restated!r}"


def test_invocations_come_from_gauntlet_dispatch_at_the_ref() -> None:
    text = _door()
    resolve = _section(text, "## Resolve the artifact", "**Filter to the run's lanes.**")
    assert "gauntlet dispatch \\" in resolve
    ...
    assert "contract_version" not in text, "the invocation is gauntlet's, never hand-inlined"
...
def test_every_posture_judge_is_a_gauntlet_dispatch_and_the_standard_column_is_gone() -> None:
    ...
    assert "Standard" not in header, "gauntlet dispatch resolves standards; the table must not mirror the charter"
...
def test_compile_runs_gauntlet_report_per_lane_and_restates_no_ingest_rule() -> None:
    text = _door()
    ...
    assert '--findings "$scratch/lane/$judge" --expect "$judge"' in compile_section, "one report run per lane"
