...
contract-v1 invocation built by `gauntlet dispatch`; the compile step reads findings
documents through `gauntlet report` before this repo's own compile rules apply. These pin
studious's own wiring around those two commands. Which flags and judge names gauntlet
accepts is `test_gauntlet_contract.py`'s; the contract version is checked at run time by
`studious gauntlet-contract` (`tests/jig/test_gauntlet_contract_verb.py`).
"""
...
def test_locate_step_runs_the_one_contract_check_and_relays_its_stop() -> None:
    """#441: discovery and the contract-version check are one verb; the door names it and
    relays its line, restating neither the lookup nor the install command."""
    text = _door()
    locate = " ".join(text[text.index("## Locate gauntlet"):text.index("## Establish the changeset")].split())
    assert "Run `studious gauntlet-contract` once, before the first dispatch" in locate
    assert "relay its one `error:` line verbatim and dispatch nothing" in locate
    for restated in ("command -v gauntlet", "/plugin install", "GAUNTLET_ROOT", "gauntlet:where"):
        assert restated not in locate, f"the locate step restates the check: {restated!r}"
    assert text.count("gauntlet-contract") == 1, "the check is run from one place"


def test_doctor_checks_the_contract_verb_beside_the_agents() -> None:
    ...
    assert "`studious gauntlet-contract`" in row, "the discovery check is the second thing a dispatch needs"
    assert "If the judges are absent or the check exits non-zero: **Critical**" in row
...
def test_lane_eight_not_installed_path_is_a_task_in_the_same_batch_and_installed_path_stays_inline() -> None:
    ...
    no Task, and the judge is dropped from `$keep` so `gauntlet report` expects no document."""
    text = _door()
...
def test_evidence_log_is_deduped_once_before_dispatch_and_passed_as_receipts_path() -> None:
    ...
    and the file reaches `gauntlet dispatch` as `--receipts-path` — the wiring, not the primitive."""
    text = _door()
    ...
    assert '${evidence_file:+--receipts-path "$evidence_file"}' in build, "the dedupe file is not what gauntlet dispatch receives"
...
def test_invocations_come_from_gauntlet_dispatch_and_are_handed_over_verbatim() -> None:
    text = _door()
    assert "gauntlet dispatch \\" in text
...
def test_compile_cites_gauntlet_report_and_the_compilation_rules() -> None:
    text = _door()
    compile_section = text[text.index("### Compile"):text.index("\n## Shared — record findings")]
    assert "gauntlet report --findings" in compile_section
    assert "--expect" in compile_section
    assert "reference/audit-compilation.md" in compile_section


def test_changeset_routing_defers_to_gauntlet_dispatch_and_restates_no_path_patterns() -> None:
...
def test_round_two_narrowing_honours_the_operators_flags() -> None:
    # The blocking-lanes narrowing and the jq filter are test_door_posture's.
    text = _door()
...
def test_compilation_rules_take_tiers_as_emitted_and_treat_empty_findings_as_clean() -> None:
    ...
    assert "no findings document" in died and "`gauntlet report` rejected" in died
...
def test_every_door_that_dispatches_passes_through_the_locate_step() -> None:
    """#410's rule after #441: whatever runs `gauntlet dispatch` — directly, or by following
    review.md inline — goes through "Locate gauntlet", so the contract check runs first."""
    prose = [p for d in ("commands", "skills", "agents", "hooks", "reference") for p in (REPO_ROOT / d).rglob("*.md")]
    dispatching = {
        p.relative_to(REPO_ROOT).as_posix()
        for p in prose
        if re.search(r"^\s*gauntlet dispatch\b", p.read_text(encoding="utf-8"), re.MULTILINE)
    }
    assert dispatching == {"commands/review.md", "commands/health.md"}, dispatching
    for rel in (*dispatching, "skills/build/SKILL.md", "skills/shape/SKILL.md"):
        assert "Locate gauntlet" in (REPO_ROOT / rel).read_text(encoding="utf-8"), rel
