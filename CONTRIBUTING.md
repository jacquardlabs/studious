...
- **Doors are the stages devs already know** — nine of them, declared in `reference/personas.md`: `bet`, `shape`, `build`, `review`, `ship`, `next`, `retro`, plus `setup` and `doctor`. Each names a stage from kanban, Scrum, XP, or Shape Up rather than a mechanism. Adding a tenth means adding a charter row first; the CI check and the docs both derive from that table.
