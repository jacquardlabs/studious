...
- **`gh` authenticated** — run `gh auth status`. If `gh` itself is missing, or the command exits non-zero: **Important** — "gh missing or unauthenticated: `/bet`, `/health` (its `backlog` mode), and the PR-time gate reminder's context all depend on it."
