...
- **`gauntlet` available** — the manifest's other declared dependency. Look for `gauntlet:*` judges in this session's registered agent listing (the same system-context listing section 2 reads). If absent: **Critical** — "gauntlet missing: a declared dependency did not install or register. Every `/review` judge lane is a `gauntlet:*` dispatch (#334 S1), so no review episode can run a single lane; `/retro` still runs its lanes locally until S3."
