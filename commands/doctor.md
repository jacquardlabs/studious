...
- **`gauntlet` available** — the manifest's other declared dependency. Look for `gauntlet:*` judges in this session's registered agent listing (the same system-context listing section 2 reads). If absent: **Critical** — "gauntlet missing: `/review` and `/retro` fan out to lanes that cannot register."
