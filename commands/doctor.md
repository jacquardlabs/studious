...
- **`jq` present** — run `command -v jq`. If it fails: **Critical** — "jq missing: `studious record` silently no-ops (see `bin/gate-ledger`'s own comment: 'Degrades silently when git or jq is unavailable') — no gate verdict, and no `/next` flow position, will ever be recorded."
- **`gh` authenticated** — run `gh auth status`. If `gh` itself is missing, or the command exits non-zero: **Important** — "gh missing or unauthenticated: `/bet`, `/retro`, `/health` (its `backlog` mode), and `/next`'s issue and milestone expansion all depend on it."
- **`python3` present** — run `command -v python3`. If it fails: **Critical** — "python3 missing: every build script (`plan-lint`, `design-lint`, `verify`, `status-flip`, `evidence-capture`, `evidence-freshness`, `build-report`, `worktree-setup`, `exorcise-report`) and `gauntlet-contract` are Python CLIs, so `/build` cannot lint a plan and `/build` cannot verify a single task — it would report success off nothing but the executor's own claim — and no `/review` or `/health` lane can locate gauntlet."
- **`viva` available** — a declared dependency of the plugin manifest. Resolve `$VIVA_DIR` the way `viva-write`'s launch block does (highest cached `jacquardlabs-marketplace/viva/*` carrying `server.py`) and check the entrypoints studious actually invokes: `scripts/loop.py`, `scripts/doc_types.py`, `scripts/context_refs.py`, `.claude/skills/viva-write/SKILL.md`. Missing plugin → **Critical** ("`/build` and `/shape` stop dead without viva — `/plugin install viva@jacquardlabs-marketplace`"). Plugin present but an entrypoint missing → **Critical** naming it and the installed version: the pinned surface moved, not the install. Also check `.viva-types/plan.json` at the project root — absent → **Important** ("`/build`'s plan stamp needs the repo-local plan type; run `/setup`").
- **`gauntlet` available** — a declared dependency of the plugin manifest. A dispatch needs two things, so check both: `gauntlet:*` judges in this session's registered agent listing (the same system-context listing section 2 reads — the `Task` tool's `subagent_type` resolves against it), and `studious gauntlet-contract` — the one check every door runs before a dispatch (`commands/review.md`, "Locate gauntlet"), which finds the `gauntlet` command and checks the findings contract version it speaks; report its line. If the judges are absent or the check exits non-zero: **Critical** — the check's own `error:` line when it printed one, then "gauntlet unusable: a declared dependency did not install, register, or speak this studious's findings contract. Every `/health` lane dispatches a `gauntlet:*` posture judge (`commands/health.md`'s area table) and every `/review` judge lane is a `gauntlet:*` dispatch (#334 S1, S3), so no inspection and no review episode can run a single lane. `/health backlog`, `/bet`, and `/retro outcomes` are the three dispatches still local — this row doesn't touch them."
- **`exorcist` available** — the third declared dependency. Look for `exorcist:exorcise` in this session's registered skill listing. Absent: **Important** — "exorcist missing: a declared dependency did not install — `/plugin install exorcist@jacquardlabs-marketplace`. `/build` Step 3 ends every build in a `no exorcise report written` Track note, and `/setup`'s ward and `/health simplify` stop here." Present: read the installed version the way the `viva` row resolves its install — the highest cached `jacquardlabs-marketplace/exorcist/*` carrying `commands/exorcise.md`, its `.claude-plugin/plugin.json` `version`. Older than `0.6.0`: **Important** naming that version — "exorcist <version> predates `--json` (0.6.0): `/build` Step 3 ends every build in a `no exorcise report written` Track note — `claude plugin update exorcist@jacquardlabs-marketplace`."

Report each check as **OK** when it succeeds. Git, `jq`, and `gh` are gate-side; `python3`, `viva`, and `exorcist` build-side; `gauntlet` gate- and inspection-side.

## 2. Plugin health

Every agent and skill Studious ships must actually be registered this session — a file present on disk with malformed frontmatter fails to register but leaves no error, silently costing a door a lane or a trigger: every skill but one backs its door directly (step 3 names the exception), and each of the three shipped agents is dispatched locally by name (`backlog-hygiene` via `/health backlog`, `backlog-priorities` via `/bet`, `review-outcomes` via `/retro outcomes`).

1. Locate the plugin's own shipped roster: glob `${CLAUDE_PLUGIN_ROOT}/agents/*.md` for agent names (filename minus `.md`) and `${CLAUDE_PLUGIN_ROOT}/skills/*/SKILL.md` for skill names (parent directory name). If `${CLAUDE_PLUGIN_ROOT}` doesn't resolve, locate the plugin's own `agents/` and `skills/` directories with Glob instead (same fallback `/setup` uses for templates) — don't guess a path. That local roster is what `/health backlog`, `/bet`, and `/retro outcomes` dispatch by name; `/review`'s judge lanes and `/health`'s are the session listing's `gauntlet:*` entries, which section 1's `gauntlet` row checks.
...
3. A shipped skill absent from the session's registered list is **Critical**. Every skill here backs a door directly except `task-execution-discipline`, which is model-invoked, not a door (`reference/personas.md` carries no row for it): for any other skill, name it and state the consequence: "`<name>` shipped but not registered this session — that door cannot run." For `task-execution-discipline` specifically, state instead: "`task-execution-discipline` shipped but not registered this session — the TDD-per-capability discipline it carries won't load for a fresh `/build` executor; no door is affected." A shipped agent absent from the session's registered list is **Critical**: "`<name>` shipped but not registered this session — `<the naming door>` silently runs without this lane."
4. Report the roster size as counts derived from step 1 (e.g. "3 agents, 4 skills shipped") — never hardcode a count.
...
`.studious/` accumulates one work file per feature, and nothing collects them automatically. When they pile up, `/next` with no argument stops being able to answer "what's next" — it has to ask you to pick from a list instead (#237).

`studious work-list` (one line per work file: slug, phase, branch, title) gives the raw inventory; classify each line by phase, matching `commands/next.md`'s own definition:
...
- **Retained** — phase `done`/`stopped`, and `studious work-get --slug <slug>` (never a raw file read — work files are read and written only through the ledger tool, same as `commands/next.md`'s own rule) shows at least one *measured* `scopeDelta` entry (#244), i.e. one with `unmeasured` not `true`. `studious gc` keeps such a file until its retention window lapses — `bin/gate-ledger`'s `SCOPE_DELTA_RETENTION_DAYS`, checked against the file's last write, not when its flow ended — after which plain `gc` collects it too; `studious gc --force` collects it immediately regardless. A work file whose scope-delta cohort is entirely `unmeasured` carries none of this protection and collects on the very next plain `gc`.

Classify:

- **More than 10 active work files** — **Important** — "`/next` will ask you to choose from N features instead of resuming one." Retained files never appear in that menu — they're terminal-phase — so they don't count toward this threshold on their own, but if any exist, name them too: "`studious work-get --slug <slug>` reads any of them before they go; `studious gc --force` collects M retained now, discarding each one's measured scope-delta history for good; plain `studious gc` collects whichever have already passed their `SCOPE_DELTA_RETENTION_DAYS` window (same permanent loss), and the rest once it passes."
- **1–10 active** — **OK**, with the active count (and the retained count, if nonzero).
- **A work file whose branch was merged into the default branch but still exists** — name it: `gc` resolves it to `done` (a `merge`/`MERGED` history entry) and collects it on the same run unless the scope-delta guard keeps it (#346). Squash-merged branches are not ancestors; they fall under the next bullet once deleted.
...
**Flow state**
- [OK|Important] <n active, k retained for scope-delta> — <"clean" | "`/next` will ask you to choose from n features" | "k retained for scope-delta — `studious work-get --slug <slug>` to read any before deciding; `studious gc --force` to collect now (discards their scope-delta history for good), or plain `gc` once their window lapses (same permanent loss)">
