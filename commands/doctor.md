...
- **`exorcist` available** — the third declared dependency. Look for `exorcist:exorcise` in this session's registered skill listing. Absent: **Important** — "exorcist missing: `/build` Step 3 ends every build in an `exorcise dispatch died` Track note, and `/setup`'s ward and `/health simplify` cannot run."
