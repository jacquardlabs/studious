...
- **`gauntlet` available** — the manifest's other declared dependency. Look for `gauntlet:*` judges in this session's registered agent listing (the same system-context listing section 2 reads). If absent: **Important** — "gauntlet missing: a declared dependency did not install or register. Nothing dispatches a `gauntlet:*` lane yet — `/review` and `/retro` still run every lane locally — so this degrades nothing today; it becomes Critical when #334 S1 repoints those lanes."

Report each check as **OK** when it succeeds. Git, `jq`, `gh`, and `gauntlet` are gate-side; `python3` and `viva` build-side.
