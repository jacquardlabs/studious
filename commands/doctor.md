...
- **`gh` authenticated** — run `gh auth status`. If `gh` itself is missing, or the command exits non-zero: **Important** — "gh missing or unauthenticated: `/bet`, `/retro`, `/health` (its `backlog` mode), and the PR-time gate reminder's context all depend on it."
...
3. A shipped skill absent from the session's registered list is **Critical**. Every skill here backs a door directly except `task-execution-discipline`, which is model-invoked, not a door (`reference/personas.md` carries no row for it): for any other skill, name it and state the consequence: "`<name>` shipped but not registered this session — that door cannot run." For `task-execution-discipline` specifically, state instead: "`task-execution-discipline` shipped but not registered this session — the TDD-per-capability discipline it carries won't load for a fresh `/build` executor; no door is affected." A shipped agent's consequence depends on whether any door still dispatches the local file: `backlog-hygiene` (`/health backlog`), `backlog-priorities` (`/bet`), and `review-outcomes` (`/retro outcomes`) are the three that do — if one of those is unregistered, it's **Critical**: "`<name>` shipped but not registered this session — `<the naming door>` silently runs without this lane." Every other shipped agent mirrors a `gauntlet:<judge>` that `/review` and `/health` dispatch instead (#334) and no door dispatches the local file today — an unregistered one of these is **OK**, not Critical: "`<name>` shipped but not registered this session — no door dispatches it locally, so this costs nothing while gauntlet's matching judge covers the lane."
4. Report the roster size as counts derived from step 1 (e.g. "24 agents, 4 skills shipped") — never hardcode a count.
