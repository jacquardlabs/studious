...
- **Recommend-only means propose, never modify.** Any command whose own contract declares it recommend-only — every `gate-*` command, every periodic `review-*`/`deep-review` command, and e.g. `/backlog-hygiene`, `/backlog-priorities`, `/studious-doctor` (`backlog-` is its own named category, `CONTRIBUTING.md:53`; none of the three is a gate or a review, but each says "recommend-only" in its own body text — a non-exhaustive list, not a name list to keep in sync: the predicate is what governs, whichever command declares it) — reports; it never modifies external state — issues, PRs, or files outside the bookkeeping boundary in the next bullet, in the consuming project.
