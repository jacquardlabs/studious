...
install. The manifest declares `viva`, `gauntlet`, and `exorcist` as dependencies, each
pinned to a version range (#441) — `/build` and `/shape` stop dead without viva; `/health`
dispatches nothing without gauntlet, and `/review`'s judge lanes are `gauntlet:*`
dispatches too, live since #334 S1 shipped; `/build`'s Step 3 is exorcist's `exorcise`.
