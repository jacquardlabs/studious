...
- Build it with `/plan` and `/build` (below), or with any other executor — by hand, or Superpowers. The gates step back in the supervised flow; in `/work-through` epics, dispatched workers build to `reference/worker-contract.md` and are gated like anyone else.
